...
std::vector<SearchHit> Engine::search(std::span<const float> query, int k,
                                      int nprobe) const {
...
std::vector<std::vector<SearchHit>>
Engine::search_batch(std::span<const float> queries, int k, int nprobe) const {
  int d = index_->dim();
  long long n = queries.size() / d;
  std::vector<std::vector<SearchHit>> results;
...
void Engine::train() {
  ...
  if (!ivf) {
    throw std::runtime_error(
        "Engine::train: only IVF index supports training.");
  }
  ivf->train();
}
...
void Engine::save(const std::string &path) const {
  ...
  // index state is written after the spec so load() can rebuild the exact
  // in-memory layout.
  if (flat) {
    flat->save_state(out);
    return;
...
Engine Engine::load(const std::string &path) {
  ...
  IndexSpec spec(dim, static_cast<Metric>(metric_raw),
                 static_cast<IndexKind>(kind_raw), nlist,
                 static_cast<StorageType>(storage_raw), normalize);
