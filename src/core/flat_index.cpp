    ...
    if (spec_.storage == StorageType::F32) {
        ...
        for (long long i = 0; i < n; i++) {
            ...
            if (spec_.normalize && spec_.metric == Metric::Cosine) {
                ...
                kernels::normalize(vec_mut, spec_.dim);
                vec = vec_mut;
            }
...
float FlatIndex::compute_score_int8(const float* query, const std::int8_t* db_vec, float scale) const {
    switch (spec_.metric) {
        case Metric::Cosine: {
            float sum = 0.0f;
...
void FlatIndex::save_state(std::ostream& out) const {
    ...
    if (spec_.storage == StorageType::F32) {
        ...
        if (vectors_i8_.size() / static_cast<std::size_t>(spec_.dim) != ids_.size()) {
            throw std::runtime_error("FlatIndex::save_state: ids size mismatch.");
        }
        ...
        throw std::runtime_error("FlatIndex::save_state: unsupported storage type.");
    }
...
void FlatIndex::load_state(std::istream& in) {
    if (spec_.storage == StorageType::F32) {
        ...
        throw std::runtime_error("FlatIndex::load_state: unsupported storage type.");
    }
