...
namespace spheni {
    namespace clustering {
        KMeans::KMeans(int k, int dim, int max_iters): k_(k), dim_(dim), max_iters_(max_iters) {}
        ...
        std::vector<float> KMeans::fit(std::span<const float> vectors) {
            ...
            for (int c = 1; c < k_; c++) {
                ...
                for(long long i = 0; i < n; i++) {
                    ...
                    for(int j = 0; j < c; j++) {
                        float dist = kernels::l2_squared(vec, centroids.data() + j * dim_, dim_);
                ...
                for(long long i =0; i < n; i++) {
                    ...
                    if (cumsum >= threshold) {
                        std::copy_n(vectors.data() + i * dim_, dim_, centroids.data() + c * dim_);
                        chosen[i] = true;
                        break;
                    }
                }
            ...
            for(int iter = 0; iter < max_iters_; iter++) {
                ...
                for(int c = 0; c < k_; c++) {
                    if (counts[c] > 0) {
                        ...
                        std::copy_n(vectors.data() + idx * dim_, dim_, new_centroids.data() + c * dim_);
                    }
                }
        ...
        std::vector<int> KMeans::predict(std::span<const float> vectors, std::vector<float> centroids) const {
            long long n = vectors.size() / dim_;
            std::vector<int> assignments(n);
            ...
            for(long long i = 0; i < n; i++) {
                ...
                for(int c = 0; c < k_; c++) {
                    ...
                    float dist = kernels::l2_squared(vec, centroid, dim_);
                    if (dist < min_dist) {
                        min_dist = dist;
                        best_cluster = c;
                    }
